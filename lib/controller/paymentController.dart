...
class Paymentcontroller extends GetxController {
  ...
  void getPaymentData(id) async {
    ...
    try {
      ...
      print('fetch detail transaksi: '+response.data['data'].toString());

      if (response.statusCode == 200) {
        List<Map<String, dynamic>> data = List<Map<String, dynamic>>.from(response.data['data']['cart']);
        items.value = data;
        dataTrans.value = response.data['data'];
        notrans.value = response.data['data']["transaction_number"];
        total.value =  response.data['data']["total_price"].toString();
        totalpoint.value =  response.data['data']["total_point"].toString();
